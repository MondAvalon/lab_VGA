module Keyboard (
  ...
  always @(posedge clk or negedge rstn) begin  //去抖动
    if (~rstn) begin
      ...
      if (ps2_c_filter == 4'b1111) ps2_c_f <= 1;
      else if (ps2_c_filter == 4'b0000) ps2_c_f <= 0;
      if (ps2_d_filter == 4'b1111) ps2_d_f <= 1;
      else if (ps2_d_filter == 4'b0000) ps2_d_f <= 0;
    end
  end

  // always @(negedge ps2_c or negedge rstn) begin
  //     if (~rstn) begin
  //         buffer <= 22'b0;
  //         counter <= 4'b0;
  //         key_event <= 10'b0;
  ...
  always @(negedge ps2_c_f or negedge rstn) begin
    if (~rstn) begin
      buffer <= 10'b0;
      counter <= 4'b0;
      key_event <= 11'b0;
